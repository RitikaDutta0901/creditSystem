 ...
 */
function sanitizePrefix(name, maxLen = DEFAULT_PREFIX_LEN) {
    if (!name)
        return "";
 ...
 */
function randomString(len, charset = RANDOM_CHARSET) {
    let out = "";
 ...
 */
function generateReferralCode(name, length = DEFAULT_LENGTH) {
 ...
 */
async function generateUniqueReferralCode(existsFn, name, length = DEFAULT_LENGTH, maxAttempts = 10) {
